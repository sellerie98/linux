...
extern int lge_drs_mngr_freeze_state_hal(int enable);
...
extern int lge_mplus_create_sysfs(struct device *panel_sysfs_dev);
...
error:
	...
	if (panel->lge.use_ddic_reg_backup && panel->lge.ddic_reg_backup_complete &&
		...
		pr_warn("skip ddic mode set on booting or not support!\n");
	}
...
static int cur_freeze_state = -1;
void lge_dsi_panel_init_freeze_state(int val)
{
	pr_info("update freeze state (%d)->(%d)\n", cur_freeze_state, val);
	cur_freeze_state = val;
}
EXPORT_SYMBOL(lge_dsi_panel_init_freeze_state);
...
static int lge_dsi_panel_parse_dt(struct dsi_panel *panel, struct device_node *of_node)
{
	...
	panel->lge.scaler_trigger_after_res_switched =
		of_property_read_bool(of_node, "lge,drs-mngr-scaler-trigger-after-resolution-switch");
	...
	panel->lge.use_hl_mode = of_property_read_bool(of_node, "lge,use-hl-mode");
	pr_info("use hl_mode=%d\n", panel->lge.use_hl_mode);

#if IS_ENABLED(CONFIG_LGE_DISPLAY_USE_FSC)
	panel->lge.use_u2_fsc = of_property_read_bool(of_node, "lge,use-u2-fsc");
	pr_info("use u2_fsc=%d\n", panel->lge.use_u2_fsc);
	if(panel->lge.use_u2_fsc) {
		rc = of_property_read_u32(of_node, "lge,fsc-curr-ua-u3", &tmp);
		if (rc) {
			panel->lge.fsc_u3 = DEFAULT_LGE_FSC_U3;
			pr_err("fail to parse lge,fsc-curr-ua-u3 Set to Default %d\n", panel->lge.fsc_u3);
		} else {
			panel->lge.fsc_u3 = tmp;
			pr_info("fsc_u3 %d\n", panel->lge.fsc_u3);
		}
		rc = of_property_read_u32(of_node, "lge,fsc-curr-ua-u2", &tmp);
		if (rc) {
			panel->lge.fsc_u2 = DEFAULT_LGE_FSC_U2;
			pr_err("fail to parse lge,fsc-curr-ua-u2 Set to Default %d\n", panel->lge.fsc_u2);
		} else {
			panel->lge.fsc_u2 = tmp;
			pr_info("fsc_u2 %d\n", panel->lge.fsc_u2);
		}
	}
#endif
	...
	rc = of_property_read_u32(of_node, "lge,aod-interface-data", &val);
	if (rc) {
		pr_err("fail to parse aod_interface");
		panel->lge.aod_interface = 0;
	} else {
		panel->lge.aod_interface = val;
		pr_err("aod_interface = %d\n", panel->lge.aod_interface);
	}
