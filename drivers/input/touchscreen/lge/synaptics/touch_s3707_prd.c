...
{
	return s3707_read(dev, regAddr, data, length);
}
...
static int s3707_sd(struct device *dev, char *buf)
{
	...
	upper_limit = s3707_get_limit(dev, "FULL_RAWDATA_UPPER",
	...
	lower_limit += s3707_get_limit(dev, "HIGH_RESISTANCE_LOWER",
				1, 1, &high_resistance_lower);
	upper_limit += s3707_get_limit(dev, "HIGH_RESISTANCE_UPPER",
...
static int s3707_lpwg_sd(struct device *dev, char *buf)
{
	struct s3707_data *d = to_s3707_data(dev);
	...
	lower_limit = s3707_get_limit(dev, "LPWG_FULL_RAWDATA_LOWER",
				TxChannelCount, RxChannelCount, (int *)rawcap_lower);
	upper_limit = s3707_get_limit(dev, "LPWG_FULL_RAWDATA_UPPER",
				TxChannelCount, RxChannelCount, (int *)rawcap_upper);
	lower_limit += s3707_get_limit(dev, "LPWG_JITTER_LOWER", 1, 1, &jitter_lower);
	upper_limit += s3707_get_limit(dev, "LPWG_JITTER_UPPER", 1, 1, &jitter_upper);
