...
static int sw49408_drm_notifier_callback(struct notifier_block *self,
...
#else
	sw49408_reg_read(dev, d->reg_info.r_chip_info_spi_addr + tc_version, (u8 *)&version, sizeof(version));
	sw49408_reg_read(dev, d->reg_info.r_chip_info_spi_addr + tc_product_id1, (u8 *)&product[0], sizeof(product));
	sw49408_reg_read(dev, spr_boot_st, (u8 *)&bootmode, sizeof(bootmode));
	sw49408_reg_read(dev, d->reg_info.r_pt_info_spi_addr + pt_info_fpc_type, (u8 *)&d->ic_info.fpc, sizeof(d->ic_info.fpc));
	sw49408_reg_read(dev, d->reg_info.r_pt_info_spi_addr + pt_info_lcm_type, (u8 *)&d->ic_info.lcm, sizeof(d->ic_info.lcm));
	sw49408_reg_read(dev, d->reg_info.r_pt_info_spi_addr + pt_info_lot_num, (u8 *)&d->ic_info.lot, sizeof(d->ic_info.lot));
	sw49408_reg_read(dev, chip_rev_addr, (u8 *)&d->ic_info.chip_revision, sizeof(d->ic_info.chip_revision));
#endif
...
{
	u32 data = 0;
	u32 phase_down = 0x800;

	do {
		sw49408_reg_write(dev, 0x7B, &phase_down, sizeof(phase_down));
		touch_msleep(delay);
...
int sw49408_chip_info_load(struct device* dev)
{
	...
	if ((status_val[1] >> 28) & 0x1) {
		TOUCH_E("Production Test info checksum error : %x\n",
								 status_val[1]);
		sw49408_reg_write(dev, 0xC04, &lockdown_pinfo, sizeof(lockdown_pinfo));
	}
	else
...
static int sw49408_suspend(struct device *dev)
{
	...
	TOUCH_TRACE();
	// [Bring-Up] Interrupt Disable for Suspend
...
*/
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		ts->drm_notif.notifier_call = sw49408_drm_notifier_callback;
...
#define INT_RESET_CLR_BIT	0x002006E0
/* (1 << 13)|(1 << 15)|(1 << 20)|(1 << 22) */
...
void sw49408_irq_runtime_engine_debug(struct device *dev)
{
	...
	if((d->info.debug.runtime_dbg_inttype == 1) || (d->info.debug.runtime_dbg_inttype == 3))
	{
		...
		memcpy(&ocd_debug, &d->info.debug, sizeof(ocd_debug));
...
static int __init touch_device_init(void)
{
	...
	if (touch_get_device_type() == TYPE_SW49408) {
		TOUCH_I("%s: sw49408 found!\n", __func__);
		return touch_bus_device_init(&hwif, &touch_driver);
	}

	TOUCH_I("%s: sw49408 not found.\n", __func__);
	return 0;
}
