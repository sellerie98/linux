...
static int calculate_battery_temperature(/* @Nonnull */ struct fg_chip *chip) {
	...
	if (tcomp_dtroot && tcomp_table[0] == NULL) {
...
load_tempcomp_table:
	...
	if (chip->batt_psy) {
		...
		pr_info("batt_psy is not available, returning %d(=%d+%d) as batt temp\n",
...
int override_fg_parse_for_battery_charactistics(struct fg_chip *chip, struct device_node *node)
{
	...
	rc = of_property_read_u32(node, "lge,fg-sys-standby-current", &temp);
	if (!rc)
	...
	fg_dbg(chip, FG_LGE, "Wrote [battery charactistics: cutoff_volt=%d, cutoff_curr=%d, "
		...
		chip->dt.cutoff_volt_mv, chip->dt.sys_standby_curr_ma,
