...
struct idtp9222_struct {
	...
	bool			configure_cspkt;	// whether sending CS packet or not
};
...
static bool idtp9222_set_capacity(struct idtp9222_struct* idtp9222) {
	if (idtp9222->configure_cspkt
		&& (idtp9222->opmode_type == WPC) && (idtp9222->status_capacity < 100)) {
...
static bool idtp9222_set_voltage_max(struct idtp9222_struct* idtp9222, int mV) {
	u8 voltage = -1;
	switch (mV) {
	case IDTP9222_VOLTAGE_MV_BPP:
		voltage = VOUT_V5P5;
		break;
	case IDTP9222_VOLTAGE_MV_EPP:
	default:
		voltage = VOUT_V9P0;
	}
	...
	if (idtp9222_is_onpad(idtp9222) && idtp9222->opmode_midpower) {
		...
		if (voltage != value) {
			idtp9222_reg_write(idtp9222->wlc_client, REG_ADDR_VOUT, voltage);
		}
		idtp9222_reg_read(idtp9222->wlc_client, REG_ADDR_VOUT, &value);
		pr_idt(IDT_REGISTER, "set voltage %dmV(0x%02x)\n", mV, value);
	}

	if (idtp9222->status_voltage != mV)
		idtp9222->status_voltage = mV;
...
static bool psy_set_dcin(struct idtp9222_struct* idtp9222, bool dcin) {
	if (idtp9222->status_dcin != dcin) {
		...
		if (idtp9222->status_dcin) {
			...
			idtp9222_set_voltage_max(idtp9222, idtp9222->status_voltage);
...
static int psy_property_set(struct power_supply* psy,
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		idtp9222_set_voltage_max(idtp9222, val->intval);
		break;
	default:
		break;
	}
...
/* Parse misc */
	...
	idtp9222->configure_cspkt = of_property_read_bool(dnode, "idt,configure-cspkt");
