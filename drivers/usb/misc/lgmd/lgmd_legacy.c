...
static int set_pd_moisture_detected(struct lge_moisture *moist, bool detected)
{
	struct dual_role_phy_instance *dual_role = moist->dual_role;
	const struct dual_role_phy_desc *desc = dual_role->desc;
	unsigned int val = detected ?
		DUAL_ROLE_PROP_MODE_FAULT : DUAL_ROLE_PROP_MODE_UFP;

	return desc->set_property(dual_role, DUAL_ROLE_PROP_MODE, &val);
}

static int get_pd_dr(struct lge_moisture *moist)
{
	struct dual_role_phy_instance *dual_role = moist->dual_role;
	const struct dual_role_phy_desc *desc = dual_role->desc;
	unsigned int val;
	...
	ret = desc->get_property(dual_role, DUAL_ROLE_PROP_DR, &val);
	if (ret < 0)
		return DUAL_ROLE_PROP_DR_NONE;
...
static void set_sbu_dry(struct lge_moisture *moist)
{
	if (moist->sbu_moisture) {
		...
		gpiod_direction_output(moist->sbu_sel, 1);
#endif
		set_pd_moisture_detected(moist, false);
	}
...
static void set_sbu_wet(struct lge_moisture *moist)
{
	if (!moist->sbu_moisture) {
		...
		set_pd_moisture_detected(moist, true);
...
void lge_moisture_set_moisture_enable(int enable)
{
	...
	if (moist_en == DUAL_ROLE_PROP_MOISTURE_EN_DISABLE) {
		int sbu_moisture = moist->sbu_moisture;
...
void lge_moisture_set_moisture(int force_moisture)
{
	...
	if (moist->prop_moisture_en == DUAL_ROLE_PROP_MOISTURE_EN_DISABLE) {
		...
		mutex_unlock(&moist->moist_lock);
		return;
	} else if (force_moisture == moist->prop_moisture) {
		mutex_unlock(&moist->moist_lock);
		return;
	}

	moist->prop_moisture = force_moisture;

	if (moist->prop_moisture) {
...
static void lge_moist_sbu_ov_adc_work(struct work_struct *w)
{
	...
	if (!get_vbus_present(moist) || get_pd_dr(moist) == DR_DFP) {
		count = 0;
